...
#optimizer = AdaDerivative.AdaDerivative(model.parameters(), lr=learningRate, weight_decay=weightDecay)


class AdaDerivative(Optimizer):
    r"""Implements AdaDerivative algorithm. Modified from Adam in PyTorch
    Arguments:
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-16,
        ...
        if self.weight_decouple:
            print('Weight decoupling enabled in AdaDerivative')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in AdaDerivative')
        if amsgrad:
            print('AMSGrad enabled in AdaDerivative')
